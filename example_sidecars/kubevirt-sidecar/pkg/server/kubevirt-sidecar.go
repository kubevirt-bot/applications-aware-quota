...
const (
	...
	SocketName             = "kubevirt-sidecar"
)

func Run() {
	var evaluatorConfig string
	flag.StringVar(&evaluatorConfig, "config", "", "Config to configure Evaluator resource allocation for vmi")
	flag.Parse()
	...
	s := Server{NewVirtLauncherCalculator(evaluatorConfig)}
